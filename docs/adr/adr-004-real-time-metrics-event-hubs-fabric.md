...
| Satisfaction proxy | A clearly labeled composite from sentiment, outcome, repeat-contact, and CSAT. Rename the current Power BI `NPS` measure to `SatisfactionProxy`; it must not be called NPS because actual NPS requires the standard survey question and calculation |
